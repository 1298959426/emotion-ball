...
window.EMOTION_SEED = [
  ...
  {
    id: '50', name: '被摸头', group: 'custom',
    desc: '眯眼享受，身体随摸头节奏一压一弹，脸颊泛暖粉，头顶有手掌拍摸',
    en: { name: 'Being Petted', desc: 'Eyes squint in bliss with each pat, body bounces to the rhythm, cheeks glowing warm pink, a hand pets the head' },
    transition: 300,
    pool: [2, 11, 19, 24], poolMs: [2500, 4500], blinkMs: [5000, 9000],
    /* breathe 加大让呼吸感更明显；初始色设为暖粉终态，保证缩略图快照一致 */
    body: { y: 3, rotate: -3, breathe: 0.025, color: '#F4D5C8' },
    eyes: { both: { y: 3, lookY: 4, lookX: 4, scaleX: 0.88, scaleY: 0.88 } },
    anims: [
      /* 摸头节奏：1.2s 一拍 —— 身体大幅下压弹起 */
      { target: 'body', prop: 'y',      type: 'sine', amp: 5,   period: 1200 },
      { target: 'body', prop: 'rotate', type: 'sine', amp: 4,   period: 1200, phase: 0.25 },
      { target: 'body', prop: 'scale',  type: 'sine', amp: 0.03, period: 1200, phase: 0.5 },
      /* 眼睛随拍动节奏：下压时眯眼(0.5)，弹起时睁眼(1.0) —— amp 0.25 在 0.75 基准上波动 */
      { target: 'eyes', prop: 'open',   type: 'sine', amp: 0.25, period: 1200 },
      /* 目光舒服地左右晃 */
      { target: 'eyes', prop: 'lookX',  type: 'sine', amp: 4,   period: 2400 },
      { target: 'eyes', prop: 'lookY',  type: 'sine', amp: 2,   period: 2400, phase: 0.5 }
    ],
    /* 脸颊渐变暖粉 —— 进入时从原色 1s 渐变到 #F4D5C8 */
    sequence: {
      settle: 'hold',
      frames: [
        { at: 0,    body: { color: '#F3F0EA' } },
        { at: 1000, body: { color: '#F4D5C8' } }
      ]
    }
  }
];
